...
for FILE in $FILES; do
  RPATH="${FILE#$INPUT/}" # https://stackoverflow.com/questions/16623835/remove-a-fixed-prefix-suffix-from-a-string-in-bash
  mkdir -p "$(dirname "$OUTPUT/$RPATH")"
  cp "$FILE" "$OUTPUT/$RPATH"
done
